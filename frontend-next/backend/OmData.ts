...
class OmData {
    ...
    private web3StorageApiToken = process.env.WEB3_STORAGE_API_TOKEN || "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJkaWQ6ZXRocjoweDAzQ2VmMGUxZWM2MmQxYmMzNjVGM0ZGMTEyRDU1Y0IwODFGQzQ0RGUiLCJpc3MiOiJ3ZWIzLXN0b3JhZ2UiLCJpYXQiOjE2NTczNDg0MjU4NjEsIm5hbWUiOiJEQVRBREFPIn0.LMFTddGfHq1raj0XwVhxWVN1J8JFp9XgbZCNx9XCj58"
    ...
    async addProposal(proposal: any) {
        ...
        const proposalData = new Blob([JSON.stringify(proposal)], { type: 'application/json' }) 
        const dataFileArray = [new File([proposalData], 'proposal.json')]
        const dataCid = await web3StorageClient.put(dataFileArray, { wrapWithDirectory: false })
