...
export const SendTransactionAddData = ({
  ...
  const handleCreateProposal = async (e: any) => {
   ...
   const dataUriInput = screenplayEncryptedPackage.dataUri
   const fileUriInput =screenplayEncryptedPackage.documentUri

   console.log("Encrypted Datas URI")
   console.log(dataUriInput)
   console.log(fileUriInput)
   ...
   setFileUri(fileUriInput);
    ...
    const dataTitle2 =dataTitle;
    const dataUri=dataUriInput; // set Link 1 here
    const fileUri=fileUriInput; // set Link 2 here
    const dataType=1;
    const groupId=1;
    const dataRoot = root;
    const dataSignal=bytes32signal;
    const dataNullifier=nullifierHash;
    const dataExternalNullifier=externalNullifier;
    const dataProof=proof; 




    const addDataTx = await contract.addData(
      dataTitle2,
      dataUri,
      fileUri,
      dataType,
      ...
      dataSignal,
      dataNullifier,
      dataExternalNullifier,
      dataProof
    )
    const tx = await addDataTx.wait()
    console.log(tx);
      ...
      <div className="flex flex-col">
        ...
        {proposalUri ? (
          <div className="h-16 items-center inline-flex mt-2 justify-center rounded-md border border-transparent bg-green-100 px-3 py-2 text-sm font-medium text-green-900 hover:bg-green-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-green-500 focus-visible:ring-offset-2">
            ...
            Data Added successfully created
            <p>Data URI: {proposalUri}</p>
          </div>
        ) : null}
      </div>
    </>
